...
- [x] **Electron native-module packaging** *(desktop)* — `@napi-rs/canvas` (+ its
  per-OS Skia binary packages as `optionalDependencies`) and `ws` are declared deps;
  `asarUnpack` unpacks the `.node`. Verified on Linux: `electron-builder --dir`
  collects + unpacks `@napi-rs/canvas-linux-x64-gnu/skia…node`, and the packaged
  module loads + renders a PNG. ✅
- [x] **Real assets** *(desktop)* — `assets/icon.png` (1024²) + `tray.png`/`tray@2x.png`
  generated by `scripts/generate-icons.mjs`; electron-builder derives `.ico`/`.icns`. ✅
- [ ] **Built installer on Windows** *(desktop)* — run `electron-builder` and
  smoke-run the installer on a clean Windows host (install → tray → window → mock
  mode). Needs a Windows machine (the `win32-x64-msvc` Skia binary is a Windows-only
  optional dep); the packaging config itself is done and Linux-verified.
